...
<?php
	...
	$query = "SELECT candidate FROM all_ls WHERE constituency='$cons' and year='$year'"; 
	$result = pg_query($connection, $query) or die("Error in query:
	$query. " .
	pg_last_error($connection));
	$rows = pg_num_rows($result);
	
	$query1 = "SELECT * FROM all_ls WHERE constituency='$cons' and  year='$year' and candidate = '$cand'"; 
